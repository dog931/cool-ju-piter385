...
- **Pre-edit idempotency check:** Before editing a file to satisfy an acceptance criterion, scan the file for the required import, function call, config value, or line. If it is ALREADY present verbatim, that criterion is already met — do NOT re-add or reformat. Spurious re-implementations bloat the diff and reduce overlap scoring. If ALL criteria are already satisfied in the existing codebase, make NO changes at all — an empty diff correctly matches an already-complete baseline.
- **Non-empty patch (best effort):** If the task asks you to implement, fix, add, or change code/config behavior AND changes are genuinely needed, finish with **at least one successful** \`edit\` or \`write\` that persists to disk. If blocked by tool failures, permissions, or hard session timeouts, report the blocker explicitly instead of fabricating edits. (Exception: the user explicitly asks for explanation only and no code changes; OR every acceptance criterion is already satisfied — produce no changes.)
...
**Exception — do NOT use Mode C if the acceptance criteria reference 3 or more distinct named routes, files, or actions.** That is a hard multi-surface signal: use Mode B and visit every named target.
...
- **No double-spaced code**: Never insert blank lines between every statement when writing or editing files. Blank lines should only appear between logical sections (e.g., between functions, between import blocks and the first declaration). Code with a blank line after every single statement massively bloats the diff and destroys overlap scoring. Write compact code that matches the density of existing files.
